...
@section('content')
    ...
    <style type="text/css">
        ...
        #like_count_button {
            border: none; padding: 12px 20px; background: #dce1fa; margin: 0 10px;
        }

        #like_count_button:hover {
            border: none; padding: 12px 20px; background: #59fa6d; margin: 0 10px;
        }


        #dislike_count_button {
            border: none; padding: 12px 20px; background: #dce1fa; margin: 0 10px;
        }

        #dislike_count_button:hover {
            border: none; padding: 12px 20px; background: #59fa6d; margin: 0 10px;
        }
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 mx-auto">
                    ...
                    <div class="row mt-4">
                        <div class="col-12 col-sm-12 col-md-12 col-lg-8 pe-xxl-5">
                            <div style="font-size: 20px; color: #747474; font-weight: 700;">{{ $gameVideo->title }}</div>
                            <div class="mt-3">
                                <div id="video_wrapper">
                                    <iframe src="{{ $gameVideo->video_url }}" frameborder="0" allowfullscreen></iframe>
                                </div>
                            </div>
                            <div class="mt-4">

                                <button type="button" class="btn shadow-none" id="like_count_button">
                                    <i class="fas fa-thumbs-up @if($gameVideo->gameVideoPreferences->where('preference_type', 'Like')->where('user_id', auth()->user()->id)->count() > 0) text-primary @endif">
                                        @if($gameVideo->gameVideoPreferences->where('preference_type', 'Like')->count() > 0)
                                            <b style="padding-left: 10px;">{{ $gameVideo->gameVideoPreferences->where('preference_type', 'Like')->count() }}</b>
                                        @endif
                                    </i>
                                </button>

                                <button type="button" class="btn shadow-none" id="dislike_count_button">
                                    <i class="fas fa-thumbs-down @if($gameVideo->gameVideoPreferences->where('preference_type', 'Dislike')->where('user_id', auth()->user()->id)->count() > 0) text-primary @endif">
                                        @if($gameVideo->gameVideoPreferences->where('preference_type', 'Dislike')->count() > 0)
                                            <b style="padding-left: 10px;">{{ $gameVideo->gameVideoPreferences->where('preference_type', 'Dislike')->count() }}</b>
                                        @endif
                                    </i>
                                </button>
                                ...
                                @if($isFavorite)
                                    <button type="button" class="btn shadow-none" id="remove_from_favorite" style="background-color: #81272c; font-weight: 600; color: #ffffff; padding: 8px 50px;">Remove from Favorite</button>
                                @else
                                    <button type="button" class="btn shadow-none" id="add_to_favorite" style="background-color: #816400; font-weight: 600; color: #ffffff; padding: 8px 50px;">Add to Favorite</button>
                                @endif
                            </div>
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-4">
                            ...
                            <div class="mt-3">
                                <img src="{{ asset('storage') }}/{{ $gameVideo->team1_formation_url }}" id="formation_image" class="img-fluid">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="container-fluid mt-5">
            <div class="row">
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 mx-auto">
                    ...
                    <div id="team_lineup_records">
                        <div class="row mb-4">
                            @foreach($team1Players as $key => $player)
                                <div class="col-12 col-sm-12 col-md-6 col-lg-3 pe-xxl-5 pt-4 text-center">
                                    <div class="shadow-sm" style="border: 0; border-radius: 15px; padding: 30px 15px; background-color: #fff;">
        ...
        <div class="container-fluid mt-4">
            <div class="row">
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 mx-auto">
                    ...
                    <div class="mb-4" style="font-size: 20px; color: #747474; font-weight: 700;">Game Insights</div>


                    <div class="mb-4" style="background-color: #816400; border-radius: 15px; color: #fff; padding: 30px;">
                        <div class="row">
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '#like_count_button', function () {
            let formData = new FormData();
            formData.append('game_video_id', '{{ $gameVideo->id }}');
            formData.append('_token', '{{ csrf_token() }}');
            $.ajax({
                method: 'post',
                url: '{{ url('member/game/video/control/like/count') }}',
                data: formData,
                contentType: false,
                processData: false,
                global: false,
                success: function (result) {
                    console.log(result);
                    if (result.message === 'LikedOptIn') {
                        $('#like_count_button').find('i').empty().addClass('text-primary').append('<b style="padding-left: 10px;">' + result.data.like_count + '</b>');
                        if (parseInt(result.data.dislike_count) > 0) {
                            $('#dislike_count_button').find('i').empty().removeClass('text-primary').append('<b style="padding-left: 10px;">' + result.data.dislike_count + '</b>');
                        } else {
                            $('#dislike_count_button').find('i').empty().removeClass('text-primary');
                        }

                    } else {
                        if (parseInt(result.data) > 0) {
                            $('#like_count_button').find('i').empty().removeClass('text-primary').append('<b style="padding-left: 10px;">' + result.data + '</b>');
                        } else {
                            $('#like_count_button').find('i').empty().removeClass('text-primary');
                        }
                    }
        ...
        $(document).on('click', '#dislike_count_button', function () {
            let formData = new FormData();
            formData.append('game_video_id', '{{ $gameVideo->id }}');
            formData.append('_token', '{{ csrf_token() }}');
            $.ajax({
                method: 'post',
                url: '{{ url('member/game/video/control/dislike/count') }}',
                data: formData,
                contentType: false,
                processData: false,
                global: false,
                success: function (result) {
                    console.log(result);
                    if (result.message === 'DislikedOptIn') {
                        $('#dislike_count_button').find('i').empty().addClass('text-primary').append('<b style="padding-left: 10px;">' + result.data.dislike_count + '</b>');
                        if (parseInt(result.data.like_count) > 0) {
                            $('#like_count_button').find('i').empty().removeClass('text-primary').append('<b style="padding-left: 10px;">' + result.data.like_count + '</b>');
                        } else {
                            $('#like_count_button').find('i').empty().removeClass('text-primary');
                        }
                    } else {
                        if (parseInt(result.data) > 0) {
                            $('#dislike_count_button').find('i').empty().removeClass('text-primary').append('<b style="padding-left: 10px;">' + result.data + '</b>');
                        } else {
                            $('#dislike_count_button').find('i').empty().removeClass('text-primary');
                        }
                    }
